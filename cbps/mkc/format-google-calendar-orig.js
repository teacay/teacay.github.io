...
var formatGoogleCalendar = (function() {
    ...
    var init = function(settings) {
        ...
        jQuery.getJSON(settings.calendarUrl, function(data) {
            ...
            if ($upcomingElem.children().length !== 0) {
				...
				jQuery(settings.noResults).insertBefore($pastElem);
				$upcomingElem.append('<li><span class="summary">Watch this space!</span><br /> <span class="date">More details coming soon.</span></li>');}
    ...
    var transformationList = function(result, tagName, format) {
        ...
        for (i = 0; i < format.length; i++) {
            ...
            format[i] = format[i].toString();
             if (format[i] === '*summary*') {
                output = output.concat('<span class="summary">' + summary + '</a></span>');
    ...
    var formatDateSameDay = function(dateStart, dateEnd, moreDaysEvent, isAllDayEvent, dayNames) {
        ...
        //month day, year time-time
            ...
            formattedTime = getFormattedTime(dateStart) + ' - ' + getFormattedTime(dateEnd);
			return dayNameStart + getMonthName(dateStart[1]) + ' ' + dateStart[0];
        }
	return dayNameStart + getMonthName(dateStart[1]) + ' ' + dateStart[0];
    ...
    return {
        init: function (settingsOverride) {
            var settings = {
                ...
                pastHeading: '<h2>Past events2</h2>',
				noResults: 'Check back here',
